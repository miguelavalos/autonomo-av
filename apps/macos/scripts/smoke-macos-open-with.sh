...
usage() {
  cat <<'USAGE'
Usage:
  scripts/smoke-macos-open-with.sh --app <path-to-Autonomo AV.app> [--env dev|prod]
...
smoke_base="$HOME/Library/Containers/$app_bundle_id/Data/tmp/autonomoav-macos-open-with-smoke-$(date +%Y%m%d%H%M%S)-$$"
smoke_root="$smoke_base/queue"
smoke_file="$smoke_base/open-with-smoke.pdf"
logs_file="$smoke_base/open-with-smoke.log"
...
cat <<EOF
Autonomo AV macOS Finder/Open With smoke passed.
